...
/*========================================
   Purpose: Motion Planning and Obstacle Avoidance for Multiple mobile Robots
    Author: Hadush Hailu
   Contact: had.hailu@gmail.com
...
void avoidance_algorithm_thread(){
  ...
  if(Spur_init() < 0 )
  {
    ...
    //printf("[+] Avoidance algorithm thread started. \n");
    Spur_set_vel( 0.2);
    Spur_set_accel( 1.0 );
    ...
    double time_of_decision, obstacle_speed;
    double robot_Ux,robot_Uy,obstacle_Ux,obstacle_Uy,robot_Vx,robot_Vy,obstacle_Vx,obstacle_Vy,robot_speed = 0.2;
    double time_CPA,distance_CPA,safety_distance = 0.5, robot_radius = 0.3,largest_obstacle_radius=0;
    ...
    double Previous_obstacle_pos_x,Previous_obstacle_pos_y;
    ...
    while(keepWhileLoop){ 
        ...
        //printf("[+] Avoiding algorithm accepted data:\n"); 
        //printf("  [-] data size: %lu \n", urg_vector_buffered.size());
        //printf("  [-] robot position: (%f,%f) orientation: %f \n",odom_x,odom_y,odom_theta);
      
        /*If the target is reached then stop!*/
        ...
        }else{ /*Target is not reached!*/
          
          /*There is an object on the environment*/
          if(urg_vector_buffered.size() > 0){ 

            /*Change the data from vector to point cloud*/
            ...
            if(number_of_clusters == 0){
              /*There is no any cluster percieved as obstacle.*/
              printf("# [*] No obstacle detected. Execute Global Planner. \n");

              /*After this, clusters are grouped as cluster's of different obstacle*/
              reset_kf == 1;
              ...
              if(previous_code_isnt_3 == 1 or Number_code_3 >= 10){
                Number_code_3 = 0;
                //string s = to_string(3) + " " + to_string(odom_x) + " " + to_string(odom_y) + " " + to_string(robot_radius) + " " + to_string(goal_x) + " " + to_string(goal_y) + "\n";
                //myfile<<s; 
              }else{
                Number_code_3 ++;
              }
              ...
              /*[Definition]:There is at least one cluster*/
              ...
              if(obstacle_vector.size() == 0){
                ...
                if(this_cluster_from_previous_d > 0.4){
              ...
              /* If there are ten datapoints in the queue then proceed to decision*/
              if(obstacle_vector.size() == 10){
                ...
                largest_obstacle_radius = obstacle_vector[0].r;
                //printf("1 %f %f %f\n", obstacle_vector[0].x,obstacle_vector[0].y,largest_obstacle_radius);
                for(int lr = 1; lr < obstacle_vector.size(); lr++){
                  //printf("%d %f %f %f\n", lr+1,obstacle_vector[lr].x,obstacle_vector[lr].y,obstacle_vector[lr].r);
                  if(largest_obstacle_radius < obstacle_vector[lr].r){
                      largest_obstacle_radius = obstacle_vector[lr].r;
                  }
                }

                /*=================== START KF =====================*/
                if(Number_of_decision == 1 or reset_kf == 1){
                  ...
                  /*The difference in time between the 10 clusters.*/
                  lower_t = (obstacle_vector[0].t + obstacle_vector[1].t + obstacle_vector[2].t + obstacle_vector[3].t)/4;
                  upper_t = (obstacle_vector[6].t + obstacle_vector[7].t + obstacle_vector[8].t + obstacle_vector[9].t)/4;
                
                  /*The average of x and y of the first four clusters*/
                  double lower_x = (obstacle_vector[0].x + obstacle_vector[1].x + obstacle_vector[2].x + obstacle_vector[3].x)/4;
                  double lower_y = (obstacle_vector[0].y + obstacle_vector[1].y + obstacle_vector[2].y + obstacle_vector[3].y)/4;
                 
                  /*The average of x and y of the last four clusters*/
                  double upper_x = (obstacle_vector[6].x + obstacle_vector[7].x + obstacle_vector[8].x + obstacle_vector[9].x)/4;
                  double upper_y = (obstacle_vector[6].y + obstacle_vector[7].y + obstacle_vector[8].y + obstacle_vector[9].y)/4;

                   
                  time_of_decision = (upper_t - lower_t)/1000;
                  //printf("    [-] Descision time: %f \n", time_of_decision);

                  /*Getting object and robot intial positions and their velocities*/
                  obstacle_Ux = upper_x;
                  obstacle_Uy = upper_y;
                  obstacle_Vx = (upper_x - lower_x)/time_of_decision;
                  obstacle_Vy = (upper_y - lower_y)/time_of_decision;
                  ...
                  Previous_obstacle_pos_x = obstacle_Ux;
                  Previous_obstacle_pos_y = obstacle_Uy;

                  //printf("%d %f %f %f\n\n\n", 1,lower_x,lower_y,largest_obstacle_radius);    
                }else{
                  /*Get the time of decision*/
                  lower_t = obstacle_vector[0].t;
                  upper_t = obstacle_vector[9].t;
                  time_of_decision = (upper_t - lower_t)/1000;
                  ...
                  obstacle_Ux = X_new(0);
                  obstacle_Uy = X_new(1);
                  obstacle_Vx = (obstacle_Ux - Previous_obstacle_pos_x)/dt;
                  obstacle_Vy = (obstacle_Uy - Previous_obstacle_pos_y)/dt;
                  obstacle_speed = sqrt(pow(obstacle_Vx,2) + pow(obstacle_Vy,2));
                ...
                /*========================AVOIDANCE ALGORITHM===================*/
                if((time_CPA <= 0) and (time_CPA >= 15) and (distance_CPA <= robot_radius + largest_obstacle_radius + safety_distance)){ 
                  ...
                  for(cv = 1; cv <= 2; cv++){
                    ...
                    l = largest_obstacle_radius + robot_radius + safety_distance;
                    ...
                    if( calculate_via_again == 1){
                        ...
                        l = largest_obstacle_radius + safety_distance;
                    }
                    ...
                    if(viaPoints_vector.size() > 0){
                      ...
                      printf("  # Try: %d detour/Via point found l(cylinder raduis): %f \n",cv,l);
                      ...
                      if(select_viaPoints_vector.size() > 1){
                        /*Save as code 4*/
                        string s = to_string(4) + " " + to_string(odom_x) + " " + to_string(odom_y) + " " + to_string(robot_radius) + " " + to_string(obstacle_Ux) + " " + to_string(obstacle_Uy)  + " " + to_string(largest_obstacle_radius) + " " + to_string(xs1) + " " + to_string(ys1) + " " + to_string(xs2) + " " + to_string(ys2) + " " + to_string(l) + " " + to_string(via_x) + " " + to_string(via_y)+ " " + to_string(goal_x) + " " + to_string(goal_y) + " " + s_info + "\n";
                        myfile<<s;

                      }else{
                        /*Save as code 0*/
                        string s = to_string(0) + " " + to_string(odom_x) + " " + to_string(odom_y) + " " + to_string(robot_radius) + " " + to_string(obstacle_Ux) + " " + to_string(obstacle_Uy)  + " " + to_string(largest_obstacle_radius) + " " + to_string(xs1) + " " + to_string(ys1) + " " + to_string(xs2) + " " + to_string(ys2) + " " + to_string(l) + " " + to_string(via_x) + " " + to_string(via_y)+ " " + to_string(goal_x) + " " + to_string(goal_y) + " " + s_info + "\n";
                        myfile<<s;

                      }                   
                      
                    }else{
                      printf("  # Try: %d Stop crossing Point Not Detected! l(cylinder raduis): %f\n",cv,l);
                      ...
                      if(cv == 1){ 
                        ...
                        /* Log information into a file */ 
                        string s = to_string(1) + " " + to_string(odom_x) + " " + to_string(odom_y) + " " + to_string(robot_radius) + " " + to_string(obstacle_Ux) + " " + to_string(obstacle_Uy)  + " " + to_string(largest_obstacle_radius) + " " + to_string(goal_x) + " " + to_string(goal_y) + " " + to_string(l) + "\n";
                        myfile<<s;
                      }   
                    }
                  ...
                  /* Log information into a file  */
                  //string s = to_string(2) + " " + to_string(odom_x) + " " + to_string(odom_y) + " " + to_string(robot_radius) + " " + to_string(obstacle_Ux) + " " + to_string(obstacle_Uy)  + " " + to_string(largest_obstacle_radius) + " " + to_string(goal_x) + " " + to_string(goal_y) + "\n";
                  //myfile<<s; 
                }/*End of else*/
                
                obstacle_vector.clear();
              }//End of Obstacle vector size is 10
...
int main(int argc, char** argv){
  ...
  /*Let the robot move to (6,0)*/
  Spur_line_GL(6.0, 0.0, 0.0);
  ...
  while(!gIsShuttingDown) { 
    ...
    if(ret > 0){
      ...
      for(i = 0 ; i < urg_data->size ; i++){
        ...
        if(x_translated >= odom_x and x_translated <= odom_x + 4 and y_translated >= odom_y - 2 and y_translated <= odom_y + 2){
